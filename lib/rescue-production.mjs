...
const identityApi="https://apiz.ebay.com";
const media="https://apim.ebay.com";
export async function ebayJson(token,path,init={},fetchImpl=fetch,baseUrl=api){
  const response=await fetchImpl(baseUrl+path,{...init,headers:{authorization:`Bearer ${token}`,"content-type":"application/json","content-language":"en-US","accept-language":"en-US",...(init.headers||{})}});
...
export async function verifySeller(token,fetchImpl=fetch){
  const user=await ebayJson(token,"/commerce/identity/v1/user/",{},fetchImpl,identityApi);
...
export async function listInventoryOffers(token,fetchImpl=fetch){
  const inventoryItems=await allPages(token,"/sell/inventory/v1/inventory_item","inventoryItems",fetchImpl),offers=[];
  for(const item of inventoryItems){
    if(!item?.sku)continue;
    offers.push(...await readOffersBySku(token,String(item.sku),fetchImpl));
  }
  return offers;
}
