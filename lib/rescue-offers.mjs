...
export async function publishCanaryOffer({token,manifestHash,approvedHash,sku,offerId,fetchImpl=fetch}){
  if(!approvedHash||approvedHash!==manifestHash)throw new Error("Canary approval hash does not match the sealed manifest.");
  const before=oneOffer(await readOffersBySku(token,sku,fetchImpl));
  if(!before||String(before.offerId)!==String(offerId))throw new Error("Canary offer identity no longer matches the verified draft.");
  if(before.listing?.listingId)return{state:"ALREADY_LIVE_RECONCILED",listingId:String(before.listing.listingId),offer:before};
  try{
    const published=await ebayJson(token,`/sell/inventory/v1/offer/${encodeURIComponent(offerId)}/publish`,{method:"POST"},fetchImpl);
    const after=oneOffer(await readOffersBySku(token,sku,fetchImpl));
    const listingId=after?.listing?.listingId||published?.listingId;
    if(!listingId)throw new Error("PUBLISHING_UNKNOWN: eBay returned success but no listing identity could be reconciled.");
    return{state:"LIVE_RECONCILED",listingId:String(listingId),offer:after||before};
  }catch(error){
    const reconciled=oneOffer(await readOffersBySku(token,sku,fetchImpl));
    if(reconciled?.listing?.listingId)return{state:"LIVE_RECONCILED_AFTER_AMBIGUOUS_RESPONSE",listingId:String(reconciled.listing.listingId),offer:reconciled};
    throw Object.assign(new Error("PUBLISHING_UNKNOWN: canary publish had no definitive response and no live listing was observed. Do not retry."),{cause:error});
  }
}
