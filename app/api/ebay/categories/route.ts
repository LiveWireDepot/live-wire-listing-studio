...
export async function GET(request:Request){
  try{
    ...
    const suggestions=(data.categorySuggestions??[]).slice(0,8).map((item:any)=>({categoryId:String(item.category?.categoryId||""),categoryName:String(item.category?.categoryName||""),path:(item.categoryTreeNodeAncestors??[]).map((node:any)=>node.categoryName).filter(Boolean).reverse()})).filter((item:any)=>item.categoryId&&item.categoryName);
