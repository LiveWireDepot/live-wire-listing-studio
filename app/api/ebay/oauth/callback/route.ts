...
export async function GET(request:Request){
  ...
  try{
    ...
    return new Response(page(`eBay ${config.environment==="production"?"Production":"Sandbox"} connected`,`Your ${config.environment==="production"?"Production":"Sandbox"} seller account is securely connected. Return to the studio to continue.`),{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","set-cookie":clearStateCookie}});
