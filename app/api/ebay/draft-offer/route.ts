import {accessToken,ebayConfig,ebayJson,requireLeafCategory} from "../../../../lib/ebay";
...
export async function POST(request:Request){
  try{
    ...
    return Response.json({created:true,sku,offerId:offer.offerId,published:false,imageCount:imageUrls.length,imageUrls,aspectCount:Object.keys(aspects).length,categoryId:String(body.categoryId),price:price.toFixed(2),createdAt:new Date().toISOString()});
...
export async function PATCH(request:Request){
  try{
    ...
    return Response.json({updated:true,offerId,categoryId,published:false},{headers:{"cache-control":"no-store"}});
