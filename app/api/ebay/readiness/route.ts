...
export async function GET(request:Request){
  try{
    ...
    return Response.json({...result,ready:Boolean(result.paymentPolicies.length&&result.fulfillmentPolicies.length&&result.returnPolicies.length&&result.locations.length),error:failures.length?`Your Sandbox account is connected, but ${failures.length} setup check${failures.length===1?"":"s"} failed. Open the missing setup guide below.`:undefined});
  }catch(error){
